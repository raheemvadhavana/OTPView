...
public class RVOTPView: UIView {
    ...
    var aciveTextField:RVOTPTextField?
    ...
    public func initializeUI() {
        ...
        aciveTextField = (viewWithTag(1) as? RVOTPTextField)
        ...
        self.aciveTextField = viewWithTag(1) as? RVOTPTextField
    ...
    @objc func getResponder(){
        if hasEnteredAllOTP && aciveTextField?.tag != self.otpTextFieldsCount{
            (viewWithTag(self.otpTextFieldsCount) as? RVOTPTextField)?.becomeFirstResponder()
        }else{
            aciveTextField?.becomeFirstResponder()
        }
    ...
    fileprivate func initializeotpTextFields() {
        ...
        for index in stride(from: 0, to: otpTextFieldsCount, by: 1) {
            let oldotpTextField = viewWithTag(index + 1) as? RVOTPTextField
    ...
    fileprivate func getotpTextField(forIndex index: Int) -> RVOTPTextField {
        ...
        let otpTextField = RVOTPTextField(frame: fieldFrame)
    ...
    fileprivate func calculateEnteredOTPSTring(isDeleted: Bool) {
        if isDeleted {
            ...
            for index in stride(from: 0, to: otpTextFieldsCount, by: 1) {
                var otpTextField = viewWithTag(index + 1) as? RVOTPTextField
...
extension RVOTPView: UITextFieldDelegate {
    public func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        (textField as? RVOTPTextField)?.shapeLayer.fillColor = otpTextFieldActiveBorderColor.cgColor
        (textField as? RVOTPTextField)?.shapeLayer.strokeColor = otpTextFieldActiveBorderColor.cgColor
        (textField as? RVOTPTextField)?.shapeLayer.lineWidth = otpTextFieldActiveBorderWidth
        self.aciveTextField = textField as? RVOTPTextField
    ...
    public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if replacedText.count >= 1 {
            ...
            (textField as! RVOTPTextField).shapeLayer.fillColor = otpTextFieldDefaultBorderColor.cgColor
            (textField as! RVOTPTextField).shapeLayer.strokeColor = otpTextFieldDefaultBorderColor.cgColor
            (textField as! RVOTPTextField).shapeLayer.lineWidth = otpTextFieldBorderWidth
            
            let nextotpTextField = viewWithTag(textField.tag + 1)
            
            if let nextotpTextField = nextotpTextField as? RVOTPTextField{
            ...
            if textField.tag > 1 && currentText.isEmpty {
                if let prevotpTextField = viewWithTag(textField.tag - 1) as? RVOTPTextField {
